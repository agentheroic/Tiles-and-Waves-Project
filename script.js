...
inputBtn.addEventListener("click", function () {
    ...
    for (let i = 0; i < order; i++) {
        ...
        if (order <= 10) {
            for (let i = 0; i < order; i++) {
                row.innerHTML += `
                <td class='w-14 h-14 m-0.5 text-center' onmouseover="play()" onmouseout="stop()"></td>
            `
            }
        }
        else if (order <= 17) {
            for (let i = 0; i < order; i++) {
                row.innerHTML += `
                <td class='w-8 h-8 m-0.5 text-center' onmouseover="play()" onmouseout="stop()"></td>
            `
            }
        }
        else if(order <= 34){
            for (let i = 0; i < order; i++) {
                row.innerHTML += `
                <td class='w-4 h-4 m-px text-center' onmouseover="play()" onmouseout="stop()"></td>
            `
            }
        }
        else {
            for (let i = 0; i < order; i++) {
                row.innerHTML += `
                <td class='w-2 h-2 m-px text-center' onmouseover="play()" onmouseout="stop()"></td>
            `
            }
        }
